...
export default function GreekFlashcards({
	...
	const [sessionCards, setSessionCards] = useState<GreekVocab[]>([])
	...
	const SETTINGS_KEY = 'greekFlashcardsSrsSettings'
	...
	const cardById = useMemo(() => {
		const map = new Map<number, GreekVocab>()
	...
	useEffect(() => {
		...
		let isMounted = true
		fetch(`/api/flashcards/settings?courseId=${courseId}&language=el`)
			.then((res) => res.json())
			.then((payload) => {
				if (!isMounted) return
	...
	const fetchSession = useCallback(async () => {
		...
		try {
			const res = await fetch(
				`/api/flashcards/session?courseId=${courseId}&language=el&limit=${sessionSize}&newRatio=${newRatio}`
			)
			const payload = await res.json()
			const rawRows = payload.cards ?? []
			const nextCards = rawRows
				.map((row: { cardId: number }) => cardById.get(row.cardId))
				.filter(Boolean) as GreekVocab[]
