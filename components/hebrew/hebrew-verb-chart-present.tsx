...
import {
	PresentConjugationTable,
	PresentConjugationRow,
} from '@/types/hebrew/verbs'

type Props = {
	data: PresentConjugationTable | null
}

export default function HebrewPresentVerbChart({ data }: Props) {
	if (!data || !data.rows || data.rows.length === 0)
		return (
			<p className="text-center text-neutral-500 mt-4">
				No present conjugation data found.
			</p>
		)

	const renderHebrew = (row?: PresentConjugationRow) =>
		row ? (
	...
	const renderTranslit = (row?: PresentConjugationRow) =>
		row ? (
	...
	const renderEnglish = (row?: PresentConjugationRow) =>
		row ? (
			...
			<table className="w-full text-center border-collapse">
				...
				<tbody>
					<tr className="odd:bg-white even:bg-neutral-50">
						<td className="border font-semibold p-2 text-sm">Present</td>
